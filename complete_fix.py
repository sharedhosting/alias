#!/usr/bin/env python3
"""
完整修复版：将MySQL dump文件转换为SQLite兼容的SQL文件
"""
...
def convert_mysql_to_sqlite_sql(input_file, output_file):
    ...
    # 枚举和集合类型
    content = re.sub(r'\bENUM\([^)]+\)\b', 'TEXT', content, flags=re.IGNORECASE)
    content = re.sub(r'\benum\([^)]+\)\b', 'TEXT', content, flags=re.IGNORECASE)
    content = re.sub(r'\bSET\([^)]+\)\b', 'TEXT', content, flags=re.IGNORECASE)
    
    # 第四步：处理 AUTO_INCREMENT
